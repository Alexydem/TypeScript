...
export function getPlaceInfo(searchParams: SearchFormData) {

  return fetch(api)
    .then((response) => response.json())
    .then<PlaceResponse>((placesData) => {
      ...
      function toggleFavoriteItem(event: Event): void {
        const clickedElement = event.target as HTMLElement;
        clickedElement.classList.toggle('active')

        const hotelId: number = Number(event.target.getAttribute('key'))


        Object.values(placesData.places).forEach((place: Place) => {
          if (place.id === hotelId) {
            ...
            console.log("Массив favorite отелей в LocalStorage",
              JSON.parse(localStorage.getItem("favoriteItems")),
              'длинна массива', JSON.parse(localStorage.getItem("favoriteItems")).length)
      ...
      Object.values(placesData.places).forEach((place: Place) => {
        let id: number = place.id
        let name: string = place.name
        let description: string = place.description
        let image: string = place.image;
        let remoteness: number = place.remoteness
        let price: number = place.price
